...
def _feature_image(event: SgahEvent) -> np.ndarray:
    state = {"signal_matrix": event.signal_matrix, "sampling_rate_hz": 1.0, "sensor_positions": list(SGAH_CHANNEL_NAMES)}
...
def _threshold_metrics(probabilities: np.ndarray, labels: np.ndarray, threshold: float) -> dict:
    ...
    return {"threshold": float(threshold), "recall": float(recall), "specificity": float(specificity), "balanced_accuracy": float(0.5 * (recall + specificity)), "accuracy": float((tp + tn) / max(len(labels), 1)), "tp": tp, "fn": fn, "fp": fp, "tn": tn}


def _calibrate_dev_threshold(estimator, dev_x: np.ndarray, dev_y: np.ndarray, dev_class_ids: np.ndarray) -> tuple[float, dict]:
    probabilities = estimator.predict_proba(dev_x)[:, 1]
    candidates = np.unique(np.concatenate(([0.0, 0.5, 1.0], probabilities)))
    rows = []
    for threshold in candidates:
        row = _threshold_metrics(probabilities, dev_y, threshold)
        predicted = probabilities >= float(threshold)
        normal = dev_class_ids == 5
        competing = np.isin(dev_class_ids, [1, 2, 3])
        row["normal_false_positive_rate"] = float(np.mean(predicted[normal])) if np.any(normal) else 0.0
        row["competing_fault_false_positive_rate"] = float(np.mean(predicted[competing])) if np.any(competing) else 0.0
        rows.append(row)
...
class _FixedSgahClassifier:
    def __init__(self, estimator, threshold: float):
        self.estimator = estimator; self.threshold = float(threshold)
    ...
    def __call__(self, feature_image):
        ...
        return {"label": "main_transformer_fault" if probability >= self.threshold else None, "confidence": max(probability, 1.0 - probability), "probabilities": {"main_transformer_fault": probability, "not_transformer_fault": 1.0 - probability}}
...
def _binary_metrics(rows: list[SgahCase]) -> dict:
    ...
    recall = None if tp + fn == 0 else tp / (tp + fn); specificity = None if tn + fp == 0 else tn / (tn + fp); precision = None if tp + fp == 0 else tp / (tp + fp)
    f1 = None if precision is None or recall is None or precision + recall == 0 else 2 * precision * recall / (precision + recall)
    balanced = None if recall is None or specificity is None else 0.5 * (recall + specificity)
    covered = sum(not r.abstained for r in rows); supported = sum(r.verification_status == "SUPPORTED" for r in rows)
    return {"case_count": len(rows), "positive_count": tp + fn, "negative_count": tn + fp, "true_positive": tp, "false_negative": fn, "false_positive": fp, "true_negative": tn, "recall": recall, "specificity": specificity, "precision": precision, "f1": f1, "balanced_accuracy": balanced, "coverage": None if not rows else covered / len(rows), "abstention_rate": None if not rows else 1.0 - covered / len(rows), "verification_support_rate": None if not rows else supported / len(rows), "mean_runtime_seconds": None if not rows else float(np.mean([r.runtime_seconds for r in rows]))}


def run_sgah_transformer_benchmark(data_dir: str | Path, *, output_dir: str | Path | None = None) -> dict:
    root = Path(data_dir); train_events = []; dev_events = []; test_events = []; manifest = []
    for class_id, label in SGAH_CLASSES.items():
        events = load_sgah_events(root / f"{class_id}-data.csv", class_id); train, dev, test = _split(events)
        if not train or not dev or not test: raise ValueError(f"SGAH class {class_id} does not have enough whole events for frozen split")
        train_events.extend(train); dev_events.extend(dev); test_events.extend(test)
    ...
    train_x = np.stack([_feature_image(event).reshape(-1) for event in train_events]); train_y = np.asarray([event.class_id == 4 for event in train_events], dtype=int)
    dev_x = np.stack([_feature_image(event).reshape(-1) for event in dev_events]); dev_y = np.asarray([event.class_id == 4 for event in dev_events], dtype=int); dev_class_ids = np.asarray([event.class_id for event in dev_events], dtype=int)
    ...
    classifier_threshold, dev_calibration = _calibrate_dev_threshold(estimator, dev_x, dev_y, dev_class_ids)
    classifier = _FixedSgahClassifier(estimator, classifier_threshold)
    train_threshold_metrics = _threshold_metrics(estimator.predict_proba(train_x)[:, 1], train_y, classifier_threshold)

    cases = []; failures = []; pipeline = TransformerDiagnosticPipeline()
    for event in test_events:
        try:
            started = perf_counter(); result = pipeline.run(event.signal_matrix, sampling_rate_hz=1.0, sensor_positions=list(SGAH_CHANNEL_NAMES), trained_image_model=classifier, anomaly_threshold=0.0); runtime = perf_counter() - started
    ...
    normal_rows = [row for row in cases if row.class_id == 5]; competing_rows = [row for row in cases if row.class_id in (1, 2, 3)]
    summary = _binary_metrics(cases); summary.update({"normal_false_positive_rate": None if not normal_rows else sum(row.predicted_transformer_fault for row in normal_rows) / len(normal_rows), "competing_fault_false_positive_rate": None if not competing_rows else sum(row.predicted_transformer_fault for row in competing_rows) / len(competing_rows)})
    ...
    payload = {
        ...
        "protocol": {
            "upstream_commit": "bbe1020e3fade83f7861657bb3eaea41c25ec0c9", "samples_per_event": 100,
            "split": "Per class, contiguous whole-event 60% train / 20% development / 20% frozen test; no row-level splitting.",
            "positive_class": "main_transformer_fault (class 4)", "negative_controls": "classes 1,2,3 competing grid faults plus class 5 normal",
            "classifier": "RandomForestClassifier(n_estimators=400, min_samples_leaf=2, class_weight=balanced_subsample, random_state=0)",
            "threshold_calibration": "Development split only: require transformer recall >=0.60, normal FPR <=0.15, competing-fault FPR <=0.30 when feasible; among feasible thresholds maximize balanced accuracy, then recall, then highest threshold. Frozen test labels are excluded.",
            ...
            "sampling_rate": "normalized 1.0 sample unit", "public_execution": "Final decisions are emitted by TransformerDiagnosticPipeline in classifier-led electrical mode; impulsiveness remains verification evidence rather than a mandatory gate."
        },
        "manifest": manifest,
        "development": {"classifier_threshold": classifier_threshold, "train_at_calibrated_threshold": train_threshold_metrics, "dev_at_calibrated_threshold": dev_calibration},
        "summary": summary, "by_class": by_class, "cases": [asdict(row) for row in cases], "failures": failures,
    }
